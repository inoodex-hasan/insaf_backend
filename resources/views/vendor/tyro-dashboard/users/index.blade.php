                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach($users as $listUser)
                            <tr>
                                <td>
                                    <a href="{{ route('tyro-dashboard.users.edit', $listUser->id) }}" class="user-cell"
                                ...
                                <td>
                                    <div class="action-buttons" style="justify-content: flex-end;">
                                        <a href="{{ route('tyro-dashboard.users.edit', $listUser->id) }}" class="action-btn"
                                            title="Edit">
                                            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        @if(method_exists($listUser, 'isSuspended') && $listUser->isSuspended())
                                            <form action="{{ route('tyro-dashboard.users.unsuspend', $listUser->id) }}" method="POST"
                                                ...
                                                @csrf
                                                <button type="submit" class="action-btn" title="Unsuspend"
                                        ...
                                        @elseif(method_exists($listUser, 'suspend'))
                                            <button type="button" class="action-btn action-btn-danger" title="Suspend"
                                                onclick="openSuspendModal({{ $listUser->id }}, '{{ addslashes($listUser->name) }}')">
                                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                        ...
                                        @if($listUser->id !== $user->id)
                                            <form action="{{ route('tyro-dashboard.users.destroy', $listUser->id) }}" method="POST"
                                                style="display: inline;">
                                                @csrf
                                                @method('DELETE')
            ...
            <form id="suspendForm" method="POST">
                ...
                <div class="modal-body">
                    <p style="margin-bottom: 1rem; color: var(--muted-foreground);">
                        You are about to suspend <strong id="suspendUserName"></strong>. This will revoke all their active
                        sessions.
                    </p>
                    <div class="form-group">
                        ...
                        <textarea id="reason" name="reason" class="form-textarea" rows="3"
                            placeholder="Enter a reason for suspension..."></textarea>
                    </div>
                </div>
