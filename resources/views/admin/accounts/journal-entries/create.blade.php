...
@section('content')
    ...
    <form action="{{ route('admin.journal-entries.store') }}" method="POST"
          x-data="journalForm()" 
        ...
        <div class="panel mt-6">
            <div class="grid grid-cols-1 gap-5 md:grid-cols-4">
                <div class="form-group">
                    ...
                    @error('date') <span class="text-danger text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="form-group">
                    ...
                    @error('period_id') <span class="text-danger text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="form-group">
                    ...
                    @error('currency_id') <span class="text-danger text-xs">{{ $message }}</span> @enderror
                </div>
                <div class="form-group">
                    <label for="reference_number">Reference # (Voucher No)</label>
                    <input type="text" name="reference_number" id="reference_number" class="form-input" placeholder="Auto-generated" />
                </div>
            </div>

            <div class="mt-5">
                <label for="note">Voucher Remarks</label>
                <textarea name="note" id="note" class="form-textarea" rows="2" placeholder="General description of the transaction..."></textarea>
            </div>
        </div>
                ...
                <tbody>
                    <template x-for="(item, index) in items" :key="index">
                        <tr class="border-b transition-colors hover:bg-white-light/10">
                            <td class="p-2">
                                <select :name="`items[${index}][chart_of_account_id]`" class="form-select text-xs" x-model="item.chart_of_account_id" required>
                            ...
                            <td class="p-2">
                                <input type="text" :name="`items[${index}][description]`" class="form-input text-xs" x-model="item.description" placeholder="Row description..." required />
                            </td>
                            <td class="p-2">
                                <input type="number" :name="`items[${index}][debit]`" step="0.01" class="form-input text-right font-mono" x-model.number="item.debit" @input="calculateTotals" placeholder="0.00" />
                            </td>
                            <td class="p-2">
                                <input type="number" :name="`items[${index}][credit]`" step="0.01" class="form-input text-right font-mono" x-model.number="item.credit" @input="calculateTotals" placeholder="0.00" />
                            </td>
                            <td class="p-2 text-center">
                                <button type="button" @click="removeItem(index)" class="text-danger hover:opacity-70" x-show="items.length > 2">
                ...
                <tfoot>
                    ...
                    <tr :class="difference === 0 && totalDebit > 0 ? 'bg-success/5' : 'bg-danger/5'">
                        ...
                        <td colspan="2" class="p-3 text-center text-lg" 
                            ...
                            <span class="text-[10px] block uppercase opacity-70" x-text="difference === 0 ? 'Entry is balanced' : 'Entry is unbalanced'"></span>
                        </td>
                        <td></td>
                    </tr>
                </tfoot>
    ...
    <script>
        function journalForm() {
            return {
                ...
                addItem() {
                    this.items.push({ chart_of_account_id: '', description: '', debit: 0, credit: 0 });
                },
